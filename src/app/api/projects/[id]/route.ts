...
async function getOwnedProject(userId: string, projectId: string) {
  const { rows } = await pool.query(
    `SELECT * FROM projects WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL LIMIT 1`,
    [projectId, userId]
  );
...
export async function GET(_req: Request, { params }: Params) {
  try {
    ...
    return json(await getOwnedProject(user.id, id));
  } catch (err) {
...
export async function PATCH(req: Request, { params }: Params) {
  try {
    ...
    const allowed = ["name", "description", "color", "status", "due_date", "progress"];
    const sets: string[] = [];
