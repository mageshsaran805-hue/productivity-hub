...
export function GoogleButton({ text = "Continue with Google" }: { text?: string }) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={isLoading}
      className="w-full h-11 px-4 rounded-2xl bg-white dark:bg-white/5 backdrop-blur-xl border border-border/50 hover:bg-foreground/5 hover:border-border text-foreground text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2.5 disabled:opacity-60"
    >
      {isLoading ? (
